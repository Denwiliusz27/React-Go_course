...
type PostgresDBRepo struct {
	DB *sql.DB // holds pool of connections to db
}
...
func (m *PostgresDBRepo) AllMovies(genre ...int) ([]*models.Movie, error) {
	...
	// if image is present use it, otherwise - take ''
	query := fmt.Sprintf(`
		select		
	...
	defer rows.Close() // database not stays open
...
func (m *PostgresDBRepo) UpdateMovie(movie models.Movie) error {
	...
	stmt := `
		update movies set title = $1, description = $2, release_date = $3,
